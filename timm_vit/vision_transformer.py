...
# class VisionTransformer_defense(nn.Module):
...
#         B = x.shape[0]
#         x = self.patch_embed(x) # torch.Size([bs,196,768])
#         # add DBAVT defense here
#         if(is_DBAVT_patch_drop): # patch drop
#             drop_patch_sum = int(DBAVT_patch_drop_rate * x.shape[1])
#             for i in range(x.shape[0]):
#                 idx = np.random.choice(np.arange(x.shape[1]), size=drop_patch_sum, replace=False)
#                 x[i,idx,:] = 0
#             # drop_patch_sum = 0
#             # x = torch.tensor([[[1,2,3,4],[4,5,6,7],[7,8,9,10]]]) # 1*3*4
#             # idx = np.random.choice(np.arange(x.shape[1]), size=drop_patch_sum, replace=False)
#             # x[:,idx,:] = 0
#             # print(x)

#         if(is_DBAVT_patch_shuffle): # patch shuffle
#             shuffle_patch_sum = int(DBAVT_patch_shuffle_rate * x.shape[1])
#             for i in range(x.shape[0]):
#                 idx = np.random.choice(np.arange(x.shape[1]), size=shuffle_patch_sum, replace=False)

#                 y = x[i,idx,:] # [shuffle_patch_sum, 768]
#                 y_idx = torch.randperm(y.shape[0])
#                 y = y[y_idx,:]
